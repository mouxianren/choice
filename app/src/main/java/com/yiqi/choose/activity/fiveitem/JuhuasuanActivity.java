...
public class JuhuasuanActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.youhui_juhuasuan);
        ...
        back_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                JuhuasuanActivity.this.finish();
                overridePendingTransition(R.anim.in_from_left,
                        R.anim.out_of_right);
            }
        });
        if(NetJudgeUtils.getNetConnection(JuhuasuanActivity.this)){
        ...
        home_jinxuan_listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(NetJudgeUtils.getNetConnection(JuhuasuanActivity.this)){
                    CustomProgressDialog.createDialog(JuhuasuanActivity.this,"");
                    GoodsInfo info=(GoodsInfo)goodList.get(position-1);
                    turnGoodsId=info.getNumIid();
                    ThreadPollFactory.getNormalPool().execute(new TurnUrlThread(info.getNumIid()+"",JuhuasuanActivity.this,hd));
                    ...
                    Toast.makeText(JuhuasuanActivity.this,"您的网络不给力，请检查更新!",Toast.LENGTH_LONG).show();
                }
        ...
        home_jinxuan_listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                ...
                final Picasso picasso = Picasso.with(JuhuasuanActivity.this);
                if (scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_TOUCH_SCROLL) {
                    picasso.resumeTag(JuhuasuanActivity.this);
                } else {
                    picasso.pauseTag(JuhuasuanActivity.this);
                }
            }
    ...
    private class goodsThread implements Runnable {
        ...
        public void run() {
            try {
                String url = getResources().getString(R.string.appurl) + "/goods2/cheap" + "?page=" + page+"&stamp="+ UrlUtils.getTime()+"&encode="+UrlUtils.getEncode();
    ...
    private class MyHandler extends Handler {
        private WeakReference<JuhuasuanActivity> mWeakReference = null;

        MyHandler(JuhuasuanActivity activity){
            mWeakReference = new WeakReference<JuhuasuanActivity>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            JuhuasuanActivity juhuasuanActivity = mWeakReference.get();
            if(juhuasuanActivity==null){
                return;
            }
            if(Build.VERSION.SDK_INT >= 17) {
                if (juhuasuanActivity == null || juhuasuanActivity.isDestroyed() || juhuasuanActivity.isFinishing()) {
                    return;
                }
            }else {
                if (juhuasuanActivity == null || juhuasuanActivity.isFinishing()) {
                    return;
                }
            }
            if (msg.what == 3) {
                    ...
                    if (code.equals("0")) {
                        ...
                        if (goods.equals("[]") || null == goods || goods.equals("null")) {
                            ...
                            if (goodList.size() > 0) {
                                ...
                                adapter = new MyAdapter(juhuasuanActivity);
            ...
            if (msg.what == 4) {
                try {
                    ...
                    if (code.equals("0")) {
                        ...
                        home_jinxuan_listView.stopRefresh();
                        home_jinxuan_listView.stopLoadMore();
                        Toast.makeText(juhuasuanActivity, "数据返回错误!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    home_jinxuan_listView.stopRefresh();
            ...
            if (msg.what == 7) {
                MyToast.show(juhuasuanActivity, 0,AndroidUtils.getWidth(juhuasuanActivity),
                        AndroidUtils.dip2px(juhuasuanActivity, 45), "您的网络不给力，请检查更新!");
                home_jinxuan_listView.stopRefresh();
            }
            if (msg.what == 8) {
...
//                                                   cmb.setText("复制的地址为空");
//                                               }else{
//                                                   cmb.setText("聚划算优惠券地址为："+data);
//                                               }
//                                               Toast.makeText(JuhuasuanActivity.this, "优惠券地址已粘贴!", Toast.LENGTH_LONG).show();


                        // System.out.println("data优惠="+data);
                        if(NetJudgeUtils.getNetConnection(juhuasuanActivity)){
                            if(TextUtils.isEmpty(data)){
                                AlibcTrade.show(juhuasuanActivity, new AlibcDetailPage(StringUtils.getID(turnGoodsId)), alibcShowParams, alibcTaokeParams, exParams , new DemoTradeCallback());

                            }else{
                                AlibcTrade.show(juhuasuanActivity, new AlibcPage(data), alibcShowParams, null, exParams, new DemoTradeCallback());
                                //AlibaPageDetails
                            }
                        }else{
                            Toast.makeText(juhuasuanActivity, "您的网络不给力，请检查更新!", Toast.LENGTH_LONG).show();
                        ...
                        //  Toast.makeText(juhuasuanActivity, "服务器错误!", Toast.LENGTH_LONG).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    if(NetJudgeUtils.getNetConnection(juhuasuanActivity)){
